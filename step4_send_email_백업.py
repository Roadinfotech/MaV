"""
Step 4 통합 버전 (v4.8): MaV 하이엔드 이메일 발송기 (Email Client Rendering Fix)
- 공포/탐욕 게이지: CSS 대신 HTML Table 너비(%) 비율을 이용한 100% 호환 마커 구현
- 차트 겹침 픽스: Table 열(Column) 너비 % 고정 및 차트 내부 숫자 강제 숨김
"""
...
RESEND_API_KEY = "re_eoiqHAT9_GkTqqXfFUsaooLg24ptXKUr6"
...
def get_sparkline_url(history_data, color_hex):
    ...
    chart_config = {
        ...
        "options": {
            "plugins": {
                "datalabels": {"display": False} # 차트 내부 숫자 강제 숨김
            }
        }
    }
...
def build_email_html(data):
    ...
    # --- 1. Headline ---
    main_insight = str(ai.get("narrative", "시장 데이터 분석 대기 중"))
    main_insight = main_insight.replace("\n", "<br><br>")
    
    # --- 2. Fear & Greed Gauge (Table Width 방식 적용) ---
    ...
    # 화살표가 이탈하지 않도록 최소/최대 퍼센트 제한
    arrow_pos = max(2, min(98, fg_val))
    
    # --- 3. Market & Sector Snapshot (Width 명시적 분배) ---
...
def send_email(data):
    print("\n📧 MaV 하이엔드 이메일 발송 준비")
    if not RESEND_API_KEY.strip() or "YOUR_RESEND" in RESEND_API_KEY:
        return print("  ❌ Resend API 키 설정 오류.")
